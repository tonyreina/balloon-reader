...
// --- nothing in the source reaches for another host ---------------------
console.log('\n-- the shipped files fetch nothing from anywhere else --');
{
  const files = ['index.html', 'css/style.css', ...readdirSync(resolve(repo, 'js')).map((f) => `js/${f}`)];

  // Each pattern is something the browser acts on without being asked. Ordinary links
  // a person can click are allowed: a link is inert until clicked.
  const patterns = [
  ...
  for (const file of files) {
    ...
    for (const [pattern, what] of patterns) {
      for (const match of text.matchAll(pattern)) found.push(`${file}: ${what} — ${match[0].slice(0, 60)}`);
    }
  }
  ...
  check(`every font is local (${fontUrls.length})`, fontUrls.filter((url) => /^https?:/.test(url)), []);
}
